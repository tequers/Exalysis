...
| `Courses/` | Optional local home for course folders. The tracked `Example_Course/` README describes an empty synthetic scaffold. | Keep real papers, extracted text, accepted records, candidates, and generated reports out of Git. Never assume a new course name is ignored. | Inspect `git status` before staging. Apply the existing private-data policy to each new local course folder. |
